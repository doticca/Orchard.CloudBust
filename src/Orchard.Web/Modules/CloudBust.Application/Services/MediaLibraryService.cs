...
using MediaFolder = CloudBust.Application.Models.MediaFolder;
...
namespace CloudBust.Application.Services
{
    ...
    {
        private static char[] HttpUnallowed = {'<', '>', '*', '%', '&', ':', '\\', '?', '#'};
        ...
        public bool CheckMediaFolderPermission(Permission permission, string folderPath) {
            if (_orchardServices.Authorizer.Authorize(Orchard.MediaLibrary.Permissions.ManageMediaContent)) return true;
            ...
            var isMyfolder = rootedFolderPath.StartsWith(userFolderPath);

            return _orchardServices.Authorizer.Authorize(isMyfolder ? Orchard.MediaLibrary.Permissions.ManageOwnMedia : permission);
        }
        ...
        public IEnumerable<ContentTypeDefinition> GetMediaTypes()
        {
            return _orchardServices
                ...
                .Where(contentTypeDefinition => contentTypeDefinition.Settings.ContainsKey("Stereotype") &&
                                                contentTypeDefinition.Settings["Stereotype"] == "Media")
        ...
        public IEnumerable<MediaPart> GetMediaContentItems(string folderPath, int skip, int count, string order,
            string mediaType, VersionOptions versionOptions = null)
        {
            return BuildGetMediaContentItemsQuery(_orchardServices.ContentManager, folderPath, order: order,
                    mediaType: mediaType, versionOptions: versionOptions)
                .Slice(skip, count);
        }
        ...
        public IEnumerable<MediaPart> GetMediaContentItemsRecursive(string folderPath, int skip, int count,
            string order, string mediaType, VersionOptions versionOptions = null)
        {
            return BuildGetMediaContentItemsQuery(_orchardServices.ContentManager, folderPath, true, order, mediaType,
                    versionOptions)
                .Slice(skip, count);
        }
        ...
        {
            return BuildGetMediaContentItemsQuery(_orchardServices.ContentManager, folderPath, mediaType: mediaType,
        ...
        public int GetMediaContentItemsCountRecursive(string folderPath, string mediaType,
            VersionOptions versionOptions = null)
        {
            return BuildGetMediaContentItemsQuery(_orchardServices.ContentManager, folderPath, true,
                    mediaType: mediaType, versionOptions: versionOptions)
                .Count();
        }
        ...
        public string GetUniqueFilename(string folderPath, string filename)
        {
            ...
            return Guid.NewGuid().ToString("N") + (string.IsNullOrWhiteSpace(extension) ? ".unknown" : extension);
        }
        ...
        public MediaPart ImportMedia(string relativePath, string filename, string contentType)
        {
            ...
            if (!_storageProvider.FileExists(_storageProvider.Combine(relativePath, filename))) return null;
        ...
        {
            if (_orchardServices.Authorizer.Authorize(Orchard.MediaLibrary.Permissions.ManageMediaContent)) return null;

            if (_orchardServices.Authorizer.Authorize(Orchard.MediaLibrary.Permissions.ManageOwnMedia))
            {
        ...
        public void CreateFolder(string relativePath, string folderName)
        {
            ...
            var folder = relativePath == null ? folderName : _storageProvider.Combine(relativePath, folderName);
            if (!_storageProvider.FolderExists(folder))
                _storageProvider.CreateFolder(folder);
        }
        ...
        public void DeleteFolder(string folderPath)
        {
            ...
            try
            {
                ...
                foreach (var mediaPart in mediaParts) contentManager.Remove(mediaPart.ContentItem);
        ...
        public void RenameFolder(string folderPath, string newFolderName)
        {
            ...
            try
            {
                var segments = folderPath.Split(new[] {Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar},
                    StringSplitOptions.RemoveEmptyEntries).ToArray();
                var newFolderPath = string.Join(Path.DirectorySeparatorChar.ToString(),
                    segments.Take(segments.Length - 1).Union(new[] {newFolderName}));

                var mediaParts = BuildGetMediaContentItemsQuery(_orchardServices.ContentManager, folderPath, true)
                    .List();
                foreach (var mediaPart in mediaParts)
                    mediaPart.FolderPath = newFolderPath + mediaPart.FolderPath.Substring(folderPath.Length);

                _storageProvider.RenameFolder(folderPath,
                    _storageProvider.Combine(Path.GetDirectoryName(folderPath), newFolderName));
            }
            catch (Exception)
            {
                _orchardServices.TransactionManager.Cancel();
        ...
        public void RenameFile(string folderPath, string currentFileName, string newFileName)
        {
            ...
            _storageProvider.RenameFile(_storageProvider.Combine(folderPath, currentFileName),
                _storageProvider.Combine(folderPath, newFileName));
        }
        ...
        public void MoveFile(string currentPath, string filename, string newPath, string newFilename)
        {
            ...
            _storageProvider.RenameFile(_storageProvider.Combine(currentPath, filename),
                _storageProvider.Combine(newPath, newFilename));
        }
        ...
        public void CopyFile(string currentPath, string filename, string duplicatePath, string duplicateFilename)
        {
            ...
            _storageProvider.CopyFile(_storageProvider.Combine(currentPath, filename),
                _storageProvider.Combine(duplicatePath, duplicateFilename));
        }
        ...
        public string UploadMediaFile(string folderPath, string fileName, Stream inputStream)
        {
            ...
            var filePath = _storageProvider.Combine(folderPath, fileName);
        ...
        private static IContentQuery<MediaPart> BuildGetMediaContentItemsQuery(
            IContentManager contentManager, string folderPath = null, bool recursive = false, string order = null,
            string mediaType = null, VersionOptions versionOptions = null)
        {
            var query = contentManager.Query<MediaPart>(versionOptions);

            query = query.Join<MediaPartRecord>();

            if (!string.IsNullOrEmpty(mediaType)) query = query.ForType(mediaType);
