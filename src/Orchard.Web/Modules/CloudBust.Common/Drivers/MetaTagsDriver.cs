    ...
    public class MetaTagsDriver : ContentPartDriver<MetaTagsPart>
	{
        ...
        protected override DriverResult Display(MetaTagsPart part, string displayType, dynamic shapeHelper) {
	        ...
	        if (metadata.DisplayRouteValues["blogId"] != null) {
				...
				if (int.TryParse(blogId, NumberStyles.Number, CultureInfo.InvariantCulture, out tmpId)) {
			...
			string descriptionTag = GetMetaDescription(siteMetaTags, blogMetaTags, part);
            string keywordsTag = GetMetaKeywords(siteMetaTags, blogMetaTags, part);
        ...
        protected override DriverResult Editor(MetaTagsPart part, dynamic shapeHelper) {
            ...
            if (part.ContentItem.ContentType == "Site") {
                result = result.OnGroup("MetaTags");
            }
        ...
        private static string GetMetaKeywords(MetaTagsPart siteMetaTags, MetaTagsPart blogMetaTags, MetaTagsPart pageMetaTags) {
            string result = pageMetaTags == null
                                ? null
                                : pageMetaTags.Keywords;
        ...
        private static Dictionary<string, string> GetTag1Records(MetaTagsPart siteMetaTags, MetaTagsPart blogMetaTags, MetaTagsPart pageMetaTags) {
            ...
            if (pageMetaTags != null && !string.IsNullOrEmpty(pageMetaTags.MetaTag1Name))
            {
                result.Add(pageMetaTags.MetaTag1Name, pageMetaTags.MetaTag1Value);
            }

            if (blogMetaTags != null && !string.IsNullOrEmpty(blogMetaTags.MetaTag1Name))
            {
                if (result.ContainsKey(blogMetaTags.MetaTag1Name)) {
                    if (string.IsNullOrEmpty(result[blogMetaTags.MetaTag1Name]) && blogMetaTags.MetaTag1Inherited) {
                        result[blogMetaTags.MetaTag1Name] = blogMetaTags.MetaTag1Value;
                    }
                }
                else {
            ...
            if (siteMetaTags != null && !string.IsNullOrEmpty(siteMetaTags.MetaTag1Name))
            {
                if (result.ContainsKey(siteMetaTags.MetaTag1Name))
                {
                    if (string.IsNullOrEmpty(result[siteMetaTags.MetaTag1Name]) && siteMetaTags.MetaTag1Inherited)
                    {
                        result[siteMetaTags.MetaTag1Name] = siteMetaTags.MetaTag1Value;
                    }
                }
                else
                {
        ...
        private static Dictionary<string, string> GetTag2Records(MetaTagsPart siteMetaTags, MetaTagsPart blogMetaTags, MetaTagsPart pageMetaTags)
        {
            ...
            if (pageMetaTags != null && !string.IsNullOrEmpty(pageMetaTags.MetaTag2Name))
            {
                result.Add(pageMetaTags.MetaTag2Name, pageMetaTags.MetaTag2Value);
            }
            ...
            if (blogMetaTags != null && !string.IsNullOrEmpty(blogMetaTags.MetaTag2Name))
            {
                ...
                {
                    if (string.IsNullOrEmpty(result[blogMetaTags.MetaTag2Name]) && blogMetaTags.MetaTag2Inherited)
                    {
                        result[blogMetaTags.MetaTag2Name] = blogMetaTags.MetaTag2Value;
                    }
                }
                else
                {
            ...
            if (siteMetaTags != null && !string.IsNullOrEmpty(siteMetaTags.MetaTag2Name))
            {
                if (result.ContainsKey(siteMetaTags.MetaTag2Name))
                {
                    if (string.IsNullOrEmpty(result[siteMetaTags.MetaTag2Name]) && siteMetaTags.MetaTag2Inherited)
                    {
                        result[siteMetaTags.MetaTag2Name] = siteMetaTags.MetaTag2Value;
                    }
                }
                else
                {
