...
namespace CloudBust.Common.Services {
    ...
    public class ThemePickerThemeSelector : IThemeSelector {
        ...
        public ThemeSelectorResult GetTheme(RequestContext context) {
            ...
            if (session != null &&
                (bool)(session[workContext.CurrentSite.SiteName + "CloudBust.Common.ThemePicker.UseDefault"] ?? false))
            {
            ...
            var settings = _cacheManager.Get(
                ...
                ctx => {
                    ctx.Monitor(_signals.When("CloudBust.Common.ThemePicker.SettingsChanged"));
            ...
            if (!String.IsNullOrWhiteSpace(selectedThemeRule.Zone)) {
                dynamic linkShape = _shapeFactory.Create("Foundation_ThemePicker_LinkToDefault");
