...
using CloudBust.Tenants.Services;
using CloudBust.Tenants.ViewModels;
using Orchard;
...
namespace CloudBust.Tenants.Controllers {
    [Themed]
    public class DashboardController : Controller {
        private readonly ITenantService _tenantService;
        private readonly ShellSettings _thisShellSettings;

        public DashboardController(ITenantService tenantService, IOrchardServices orchardServices, ShellSettings shellSettings) {
        ...
        public ActionResult Index(string filter) {
            ...
            //if (!IsExecutingInDefaultTenant()) return new HttpUnauthorizedResult();

            if (!string.IsNullOrWhiteSpace(filter)) {
                if (filter.ToLowerInvariant() == "running")
                    return View(new TenantsIndexViewModel {
                        TenantSettings = _tenantService.GetTenants().Where(x => x.State == TenantState.Running)
                    });
                if (filter.ToLowerInvariant() == "suspended")
                    return View(new TenantsIndexViewModel {
                        TenantSettings = _tenantService.GetTenants().Where(x => x.State == TenantState.Disabled)
                    });
                return View(new TenantsIndexViewModel {
                    TenantSettings = _tenantService.GetTenants().Where(x => x.State != TenantState.Running && x.State != TenantState.Disabled)
                });
            }
        ...
        public ActionResult Add() {
            ...
            var viewModel = new TenantAddViewModel {DataProvider = "SqlServer"};
        ...
        public ActionResult AddPost(TenantAddViewModel viewModel) {
            if (!Services.Authorizer.Authorize(StandardPermissions.SiteOwner, T("You don't have permission to create tenants."))) return new HttpUnauthorizedResult();

            if (!IsExecutingInDefaultTenant()) return new HttpUnauthorizedResult();

            if (_tenantService.GetTenants().Any(tenant => string.Equals(tenant.Name, viewModel.Name, StringComparison.OrdinalIgnoreCase))) ModelState.AddModelError("Name", T("A tenant with the same name already exists.", viewModel.Name).Text);

            // Ensure tenants name are valid.
            if (!string.IsNullOrEmpty(viewModel.Name) && !Regex.IsMatch(viewModel.Name, @"^[a-zA-Z]\w*$")) ModelState.AddModelError("Name", T("Invalid tenant name. Must contain characters only and no spaces.").Text);

            if (!string.Equals(viewModel.Name, "default", StringComparison.OrdinalIgnoreCase) && string.IsNullOrWhiteSpace(viewModel.RequestUrlHost) && string.IsNullOrWhiteSpace(viewModel.RequestUrlPrefix)) ModelState.AddModelError("RequestUrlHostRequestUrlPrefix", T("RequestUrlHost and RequestUrlPrefix can not be empty at the same time.").Text);
        ...
        public ActionResult ResetPost(TenantResetViewModel viewModel) {
            ...
            if (tenant == null)
                return HttpNotFound();
            if (tenant.Name == _thisShellSettings.Name)
                return new HttpUnauthorizedResult();
            ...
            try {
                _tenantService.ResetTenant(tenant, viewModel.DropDatabaseTables, false);
                return RedirectToAction("Index");
            }
            catch (Exception ex) {
