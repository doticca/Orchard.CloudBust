...
namespace Q42.DbTranslations.Services {
    ...
    public class LocalizationManagementService : ILocalizationManagementService {
        ...
        public IEnumerable<StringEntry> ExtractDefaultTranslation(string sitePath, string targetSitePath) {
            ...
            var corePoPath = System.IO.Path.Combine(
                "Core", "App_Data",
                "Localization", "{0}",
                "orchard.core.po");
            ...
            site.Files("module.txt", true)
                .Read((content, path) => {
                     ...
                     var poPath =
                         path.MakeRelativeTo(sitePath).Tokens[0].Equals("core", StringComparison.OrdinalIgnoreCase) ? corePoPath : GetModuleLocalizationPath(site, moduleName);
        ...
        public void SyncTranslation(string sitePath, string cultureCode) {
            Path.Get(sitePath)
                ...
                .ForEach(baselinePath => {
                     ...
                     if (path.Exists) path.Open(inStream => ReadTranslations(inStream, translations), FileMode.Open, FileAccess.Read, FileShare.Read);
                ...
                while (!reader.EndOfStream) {
                    ...
                    if (line != null) {
                        if (line.StartsWith("#: ")) {
                            ...
                            if (!translations.Contains(translation, comparer)) translations.Add(translation);
            ...
            for (var i = 0; i < str.Length; i++) {
                ...
                if (escaped) {
                    if (sb == null) {
                        ...
                        if (i > 1) sb.Append(str.Substring(0, i - 1));
                    }
