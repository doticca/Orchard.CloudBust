...
namespace MSBuild.Orchard.Tasks
{
    ...
    public class ValidateExtensionProjectFiles : Task
    {
        ...
        public class Validator
        {
            ...
            private static readonly Guid[] MvcGuids =
            {
                new Guid("{F85E285D-A4E0-4152-9332-AB1D724D3325}") /* MVC2 */, new Guid("{E53F8FEA-EAE0-44A6-8774-FFD645390401}") /* MVC3 */
            };
            ...
            {
                var document = XDocument.Load(_item.ItemSpec, LoadOptions.SetLineInfo);
            ...
            {
                var elements = document.Elements(Project).Elements(ItemGroup).Elements(None);
                ...
                foreach (var element in elements)
                {
                    ...
                    if (!isValid)
                    {
                        var message = string.Format("\"{0}\" element name for include \"{1}\" should be \"{2}\".", element.Name.LocalName, filePath, Content.LocalName);
            ...
            private static bool IsValidExcludeFile(string filePath)
            {
                ...
                return validExtensions.Contains(fileExtension, StringComparer.InvariantCultureIgnoreCase) || validFilenames.Contains(filePath, StringComparer.InvariantCultureIgnoreCase);
            }
            ...
            private void CheckCodeAnalysisRuleSet(XDocument document)
            {
                ...
                var elements = document.Elements(Project).Elements(PropertyGroup).Elements(CodeAnalysisRuleSet);

                foreach (var element in elements)
                {
                    var filename = Path.GetFileName(element.Value);
                    var isValid = StringComparer.OrdinalIgnoreCase.Equals(filename, orchardbasiccorrectnessRuleset);
                    if (!isValid)
                    {
                        var message = string.Format("\"{0}\" element should be \"{1}\" instead of \"{2}\".", element.Name.LocalName, orchardbasiccorrectnessRuleset, element.Value);
                        AddValidationError(element, message);
                    }
                }
            }
            ...
            {
                var elements = document.Elements(Project).Elements(PropertyGroup).Elements(OutputPath);

                foreach (var element in elements)
                {
                    var isValid = StringComparer.OrdinalIgnoreCase.Equals(element.Value, "bin") || StringComparer.OrdinalIgnoreCase.Equals(element.Value, "bin\\");

                    if (!isValid)
                    {
                        var message = string.Format("\"{0}\" element should be \"bin\\\" instead of \"{1}\".", element.Name.LocalName, element.Value);
                        AddValidationError(element, message);
                    }
                }
            }
            ...
            {
                var elements = document.Elements(Project).Elements(PropertyGroup).Elements(ProjectTypeGuids);
                foreach (var element in elements)
                {
                    var guids = element.Value.Split(';').Select(g => Guid.Parse(g));

                    foreach (var guid in guids)
                        if (MvcGuids.Contains(guid))
                        {
                            var message = string.Format("\"{0}\" element contains an MVC tooling Guid. " + " This prevents the project from loading in Visual Studio if MVC tooling is not installed.", ProjectTypeGuids.LocalName);
                            AddValidationError(element, message);
                        }
                }
            }
